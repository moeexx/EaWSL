...
const selectedPath = (value: unknown) =>
...
const pickImportFile = async () =>
  selectedPath(
    await open({
      title: getCopy().acquire.dialogs.importFileTitle,
      directory: false,
      multiple: false,
      filters: [
        {
          name: getCopy().acquire.dialogs.importFileFilterName,
          extensions: ["tar", "tar.gz", "tar.xz", "vhdx"],
        },
      ],
    }),
  );
...
export function createAcquireWorkspaceViewModel() {
  ...
  const importNoun = $derived(
    detectedImportKind === "archive"
      ? copy.acquire.importNouns.archive
      : detectedImportKind === "vhdx"
        ? copy.acquire.importNouns.vhdx
        : copy.acquire.importNouns.generic,
  );
  const nameProbePending = $derived(
    queryState.distros.data === null &&
      queryState.distros.activity === "loading",
  );
  const nameProbeError = $derived(
    queryState.distros.data === null &&
      !nameProbePending &&
      queryState.distros.hasError
      ? copy.acquire.validation.installedDistroListReadFailed
      : null,
  );
  ...
  const importValidation = $derived(
    validateImport({
      copy,
      draft: importDraft,
      kind: detectedImportKind,
      target: probeState.importTarget,
      fileProbe: probeState.importFile,
  ...
  const spaceNotice = $derived(
    selectedDistro === null
      ...
      : getSpaceNotice({
          ...
          target: probeState.install,
          emptyMessage: copy.acquire.validation.installSpaceEmpty,
        }),
  );
  const importSpaceNotice = $derived(
    getSpaceNotice({
      copy,
      location: importDraft.location,
      target: probeState.importTarget,
      emptyMessage: copy.acquire.validation.importSpaceEmpty,
    }),
  );
  ...
  const checkInstallLocation = (value: string) =>
    selectedDistroName === null || !value.trim()
      ? probes.resetInstallTarget()
      : void probes.checkInstallTarget(value);
  const checkImportLocation = (value: string) =>
    !value.trim()
      ? probes.resetImportTarget()
      : void probes.checkImportTarget(value);
  const setInstallLocation = (value: string, touched: boolean) => {
    installLocationTouched = touched;
    draft = { ...draft, location: value };
    checkInstallLocation(value);
  };
  const setImportLocation = (value: string, touched: boolean) => {
    importLocationTouched = touched;
    importDraft = { ...importDraft, location: value };
    checkImportLocation(value);
  };
  ...
  async function loadSettings() {
    ...
    if (selectedDistroName !== null && !installLocationTouched)
      setInstallLocation(makeTarget(next, draft.name), false);
    if (detectedImportKind === "archive" && !importLocationTouched)
  ...
  async function refreshOnlineDistros() {
    ...
    try {
      ...
      pushToast({
        ...
        title: ok
          ? copy.acquire.refresh.completedTitle
          : copy.acquire.refresh.failedTitle,
        message: ok
          ? copy.acquire.refresh.successMessage
          : copy.acquire.refresh.failedMessage,
      });
    } finally {
  ...
  function selectDistro(name: string) {
    ...
    draft = createInstallDraft(
      distro.name,
      makeTarget(defaultLocation, distro.name),
    );
    probes.resetInstallTarget();
  ...
  function updateInstallName(name: string) {
    draft = { ...draft, name };
    if (!installLocationTouched)
      setInstallLocation(makeTarget(defaultLocation, name), false);
  }

  async function chooseInstallLocation() {
    if (!hasTauriBridge()) return;
    const root = await pickDirectory(
      copy.acquire.dialogs.installLocationTitle,
  ...
  function updateImportName(name: string) {
    ...
    if (!importLocationTouched && detectedImportKind === "archive")
      setImportLocation(makeTarget(defaultLocation, name), false);
  }

  function updateImportFile(file: string) {
    const kind = detectImportKind(file);
    const name = importNameTouched
      ? importDraft.name
      : deriveDistroNameFromImportFile(file);
    const location = importLocationTouched
      ? importDraft.location
      : kind === "vhdx"
        ? (getWindowsParentPath(file) ?? "")
        : kind === "archive"
          ? makeTarget(defaultLocation, name)
          : importDraft.location;
  ...
  async function chooseImportLocation() {
    if (!hasTauriBridge()) return;
    const root = await pickDirectory(
      copy.acquire.dialogs.importRootTitle,
      importDraft.location || defaultLocation,
    );
    if (root !== null)
      setImportLocation(makeTarget(root, importDraft.name), true);
  }
  ...
  async function startInstall() {
    if (selectedDistro === null || installSubmitDisabled)
      return pushToast({
        tone: "error",
    ...
    const options: InstallOptions = {
      ...
      vhdSize: formatVhdSizeGb(draft.vhdSize),
      fixedVhd: draft.fixedVhd,
    };
    installSubmitting = true;
    try {
      if (
        await submitAcquireTask({
          operation: "install",
          distro: selectedDistro.name,
          displayName,
          location: draft.location.trim(),
          options,
        })
  ...
  async function startImport() {
    if (
      ...
      detectedImportKind === null ||
      importSubmitDisabled
    )
      return pushToast({
        tone: "error",
        title: copy.acquire.toasts.cannotStartImportTitle,
        message: copy.acquire.toasts.importValidationFallback,
      });
      ...
      const completed =
        detectedImportKind === "archive"
          ? await submitAcquireTask({
              ...
              displayName: importDraft.name.trim(),
              location: importDraft.location.trim(),
              file: importDraft.file.trim(),
            })
          : await submitAcquireTask({
              operation: "importVhd",
              displayName: importDraft.name.trim(),
              sourceVhdx: importDraft.file.trim(),
              targetDirectory: getVhdxTargetDirectory(importDraft),
            });
  ...
  $effect(() =>
    untrack(() => {
      const unQuery = queryCache.subscribe((state) => {
        ...
        if (
          selectedDistroName !== null &&
          state.onlineDistros.data &&
          findOnlineDistro(state.onlineDistros.data, selectedDistroName) ===
            null
        )
          clearInstall();
      });
      const unLongTask = longTaskState.subscribe(
        (state) => (hasActiveLongTask = state.hasActiveLongTask),
      );
  ...
  return {
    ...
    get onlineDistros() {
      return onlineDistroViews;
    },
    get onlineState() {
      return getOnlineListState(queryState.onlineDistros);
    },
    get selectedDistro() {
      return selectedDistroView;
    },
    get selectedDistroName() {
    ...
    get detectedImportKind() {
      return detectedImportKind;
    },
    get importNoun() {
      return importNoun;
    },
    ...
    get refreshDisabled() {
      return refreshing || queryState.onlineDistros.activity === "loading";
    },
    get validation() {
      return validation;
    },
    ...
    get importSpaceNotice() {
      return importSpaceNotice;
    },
    get installSubmitting() {
      return installSubmitting;
    },
    get importSubmitting() {
      return importSubmitting;
    },
    get installSubmitDisabled() {
      return installSubmitDisabled;
    },
    get importSubmitDisabled() {
      return importSubmitDisabled;
    },
    callbacks: {
      ...
      updateImportName,
      updateImportFile,
      updateImportRoot: (value: string) => setImportLocation(value, true),
      chooseImportFile,
      chooseImportRoot: chooseImportLocation,
      startImport,
    },
  };
}

export type AcquireWorkspaceViewModel = ReturnType<
  typeof createAcquireWorkspaceViewModel
>;
