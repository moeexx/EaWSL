...
import {
  ...
  joinWindowsPath,
} from "$lib/shared/windows-path";
import type { AppCopy } from "$lib/i18n";
import type {
  FileSystemPathProbeState,
  PathVolumeSpaceProbeState,
...
export const createInstallDraft = (name = "", location = ""): InstallDraft => ({
  name,
  location,
  vhdSize: "",
  fixedVhd: false,
});
export const createImportDraft = (): ImportDraft => ({
  name: "",
  file: "",
  location: "",
});
export const createDistroTargetLocation = (
  root: string,
  name: string,
): string => joinWindowsPath(root, name) ?? "";
export const formatVhdSizeGb = (value: string): string | null =>
  value.trim() ? `${value.trim()}GB` : null;
export const findOnlineDistro = (
  distros: OnlineDistro[],
  name: string | null,
) => (name ? (distros.find((distro) => distro.name === name) ?? null) : null);
export const detectImportKind = (path: string): ImportKind | null =>
  /\.(tar|tar\.gz|tar\.xz)$/i.test(path.trim())
    ? "archive"
    : /\.vhdx$/i.test(path.trim())
      ? "vhdx"
      : null;
export const deriveDistroNameFromImportFile = (path: string): string =>
  (getWindowsFileName(path) ?? "")
    .replace(/\.(tar\.gz|tar\.xz|tar|vhdx)$/i, "")
    .trim();
export const getVhdxTargetDirectory = (draft: ImportDraft): string | null =>
  isDirectVhdxImport(draft) ? null : finalLocation(draft.location);

export function getOnlineListState(
  entry: QueryEntry<OnlineDistro[]>,
): OnlineListState {
...
export function validateInstall(input: {
  copy: AppCopy;
  enabled: boolean;
  draft: InstallDraft;
  target: TargetProbe;
  nameDuplicate: boolean;
  nameProbePending: boolean;
  nameProbeError: string | null;
  ...
  const locationError = enabled
    ? validateTargetDirectory(copy, draft.location, target)
    : null;
  return {
    nameError,
    vhdSizeError,
    locationError,
    disabled:
      !input.hasTauriBridge ||
      !enabled ||
      input.nameProbePending ||
      !!nameError ||
      !!vhdSizeError ||
      !!locationError ||
      isTargetChecking(draft.location, target),
  };
}
...
export function validateImport(input: {
  ...
  target: TargetProbe;
  fileProbe: FileSystemPathProbeState;
  nameDuplicate: boolean;
  nameProbePending: boolean;
  nameProbeError: string | null;
  hasTauriBridge: boolean;
  ...
  const fileError =
    validateImportFile(copy, draft.file, input.kind) ??
    validateImportFileProbe(copy, fileProbe);
  ...
  return {
    ...
    disabled:
      ...
      !!fileError ||
      !!locationError ||
      (draft.file.trim().length > 0 && fileProbe.status === "loading") ||
      isTargetChecking(draft.location, target),
  };
}

export function getSpaceNotice(input: {
  copy: AppCopy;
  location: string;
  target: TargetProbe;
  emptyMessage: string;
}): { tone: SpaceTone; message: string } {
  if (finalLocation(input.location) === null)
    return { tone: "neutral", message: input.emptyMessage };
  const { copy, target } = input;
  const { space } = target;
  if (space.status === "loading")
    return { tone: "info", message: copy.acquire.validation.diskSpaceChecking };
  if (space.status === "error")
    return {
      tone: "error",
      message:
        space.errorMessage ?? copy.acquire.validation.diskSpaceReadFailed,
    };
  if (space.status !== "ready" || space.freeBytes === null)
    return { tone: "info", message: copy.acquire.validation.diskSpaceWaiting };
  const free = formatBytes(space.freeBytes, 2, copy.common.missing);
  return space.freeBytes <= MIN_FREE
    ? {
        tone: "error",
        message: copy.acquire.validation.diskSpaceNotEnough(free),
      }
    : {
        tone: "success",
        message: copy.acquire.validation.diskSpaceEnough(free),
      };
}
...
function validateVhdSize(copy: AppCopy, draft: InstallDraft): string | null {
  ...
  if (draft.fixedVhd && parsed.state === "empty")
    return copy.acquire.validation.fixedVhdSizeRequired;
  if (parsed.state === "invalid") return copy.acquire.validation.vhdSizeInteger;
  return parsed.bytes !== null && parsed.bytes < MIN_VHD
    ? copy.acquire.validation.vhdSizeMinimum
    : null;
}
...
function validateTargetDirectory(
  ...
  if (finalLocation(value) === null)
    return (
      options.emptyMessage ?? copy.acquire.validation.installLocationRequired
    );
  if (target.location.status === "error")
    return (
      target.location.errorMessage ??
      options.probeFailedMessage ??
      copy.acquire.validation.installLocationProbeFailed
    );
  if (
    target.location.status === "ready" &&
    target.location.probe?.exists &&
    !options.allowExistingDirectory
  )
    return (
      options.existsMessage ?? copy.acquire.validation.installLocationExists
    );
  if (target.space.status === "error")
    return (
      target.space.errorMessage ?? copy.acquire.validation.diskSpaceReadFailed
    );
  return target.space.status === "ready" &&
    target.space.freeBytes !== null &&
    target.space.freeBytes <= MIN_FREE
    ? copy.acquire.validation.diskSpaceInsufficient(
        formatBytes(MIN_FREE, 2, copy.common.missing),
      )
    : null;
}
...
function validateImportFileProbe(
  copy: AppCopy,
  probe: FileSystemPathProbeState,
) {
  if (probe.status === "idle" || probe.status === "loading") return null;
  if (probe.status === "error")
    return probe.errorMessage ?? copy.acquire.validation.importFileProbeFailed;
...
function parseDiskSize(value: string): DiskSize {
  ...
  return Number.isSafeInteger(numeric)
    ? { state: "valid", bytes: numeric * GB }
    : { state: "invalid", bytes: null };
}

function isTargetChecking(value: string, target: TargetProbe) {
  return (
    finalLocation(value) !== null &&
    (target.location.status === "loading" ||
      (target.space.status !== "ready" && target.space.status !== "error"))
  );
}
...
function isDirectVhdxImport(draft: ImportDraft) {
  ...
  return (
    detectImportKind(draft.file) === "vhdx" &&
    location.length > 0 &&
    location === sourceParent
  );
}
