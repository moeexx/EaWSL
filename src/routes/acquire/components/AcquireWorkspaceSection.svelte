<script lang="ts">
  ...
  const cardClass =
    "panel-surface flex h-full min-h-[400px] min-w-0 flex-col p-3";
</script>

{#if props.activeMode === "store"}
  <div
    class="grid auto-rows-fr grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)] items-stretch gap-[11px]"
  >
    <article class={cardClass}>
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 items-baseline gap-2">
          <h3 class="text-[1.04rem] font-semibold leading-tight text-shell-950">
            {copy.onlineList.title}
          </h3>
          <span class="text-[12px] leading-4 text-shell-500"
            >{copy.onlineList.count(model.onlineDistros.length)}</span
          >
        </div>
        <RefreshButton
      ...
      <div class="mt-2.5 flex min-h-0 flex-1 flex-col">
        <OnlineDistroList {model} />
      </div>
    </article>
    <article class={cardClass}>
      <h3 class="text-[1.04rem] font-semibold text-shell-950">
        {copy.sections.installConfig}
      </h3>
      <div class="mt-2.5 flex min-h-0 flex-1 flex-col">
        <InstallFormPanel {model} />
      </div>
    </article>
  </div>
{:else}
  <div
    class="grid grid-cols-1 items-stretch gap-[11px] xl:grid-cols-[minmax(0,760px)_minmax(300px,1fr)]"
  >
    <article class={cardClass}>
      <h3 class="text-[1.04rem] font-semibold text-shell-950">
        {copy.sections.importConfig}
      </h3>
      <div class="mt-2.5 flex min-h-0 flex-1 flex-col">
        <ImportFormPanel {model} />
      </div>
    </article>
    <aside class={cardClass}>
      <div class="flex items-center justify-between gap-3">
        <h3 class="text-[1.04rem] font-semibold text-shell-950">
          {copy.importGuide.title}
        </h3>
        <Info size={17} strokeWidth={1.9} class="shrink-0 text-shell-600" />
      </div>
