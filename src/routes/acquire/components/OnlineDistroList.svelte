<script lang="ts">
  ...
  const showLoading = $derived(
    model.refreshing ||
      model.queryState.onlineDistros.activity === "loading" ||
...
<div class="flex h-full min-h-[320px] min-w-0 flex-col gap-3">
  {#if hasDistros}
    <div class="relative min-h-0 flex-1">
      <div
        class={`grid grid-cols-2 auto-rows-max content-start items-start gap-2 min-[1100px]:grid-cols-3 ${showLoading ? "opacity-45" : ""}`}
      >
        {#each model.onlineDistros as distro (distro.name)}
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <strong
                class="block truncate text-[14px] font-semibold text-shell-950"
                >{distro.friendly_name}</strong
              >
              <p class="mt-0.5 truncate text-[12px] leading-4 text-shell-500">
                {distro.name}
              </p>
            </div>
            {#if selected}<span
      ...
      {#if showLoading}<div class={loading(true)} role="status">
          <div
            class="inline-flex items-center gap-2.5 text-[15px] font-semibold"
          >
            <LoaderCircle
              size={21}
              strokeWidth={2}
              class="animate-spin text-accent-700"
            /><span>{commonCopy.loading}</span>
          </div>
        </div>{/if}
    </div>
  {:else if showLoading || model.onlineState === "loading"}
    <div class={loading(false)} role="status">
      <div class="inline-flex items-center gap-2.5 text-[15px] font-semibold">
        <LoaderCircle
          size={21}
          strokeWidth={2}
          class="animate-spin text-accent-700"
  ...
  {:else if model.onlineState === "error" || model.onlineState === "recovering"}
    <div class={`${stateCard} border-rose-200/80 bg-rose-50/80 text-rose-700`}>
      <div>
        <p class="text-[14px] font-semibold">{commonCopy.readFailed}</p>
        <p class="mt-1 text-[12px] leading-5">{copy.readFailedMessage}</p>
      </div>
    </div>
  {:else}
    <div
      class={`${stateCard} border-shell-200/80 bg-shell-50/80 text-shell-600`}
    >
      <div>
        <p class="text-[14px] font-semibold text-shell-700">
          {copy.emptyTitle}
        </p>
        <p class="mt-1 text-[12px] leading-5">{copy.emptyMessage}</p>
      </div>
    </div>
  {/if}
</div>
